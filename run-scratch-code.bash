...
function err_trap_func () {
	exit_status="$?"
	echo "Exiting with status \"$exit_status\" due to command \"$BASH_COMMAND\" (call stack: bash script line(s) $LINENO ${BASH_LINENO[*]} in $0)" >&2
	exit "$exit_status"
}
...
if [[ ( "$#" -le "$numArgsMin"-1 ) || ( "$#" -ge "$numArgsMax"+1 ) || ( "$#" == 1 && "$1" == "--help" ) ]] ; then
	cat >&2 << EOF
Usage:
$nameOfThisProgram [ARGS...] 
EOF
	exit 1
fi

node run-walk-galloway.app.js --scratch "$@"
